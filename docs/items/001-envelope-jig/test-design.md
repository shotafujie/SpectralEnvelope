...
- **観測点**:
  - 合成に使った fo / sp / ap は `pyworld.synthesize` をスパイして観測する。記録したうえで本物に委譲する。
...
- **TC-005-1** 3 秒 wav → `frames` == `pyworld.harvest` を同じ信号にかけた fo の要素数（601）
- **TC-005-2** 包絡 API で `frame = frames − 1` → 200（最終フレームが存在する）

### SPEC-006 voiced_frames の定義

- **TC-006-1** 合成母音の前後 0.5 秒を無音にした wav → `voiced_frames` が昇順、先頭 0 付近と末尾付近のフレームを含まない、各要素のフレームで fo > 0（合成に渡る fo で確認）
- **TC-006-2** 同入力 → `voiced_frames` の要素数 == 合成に渡る fo の fo > 0 の数

### SPEC-007 f0_mean の定義

- **TC-007-1** fo ≈120Hz の合成母音 → `f0_mean` が 120 ±6
- **TC-007-2** デジタル無音 2 秒（有声フレームなし）→ `f0_mean` == 0 かつ `voiced_frames` == []
- **TC-007-3** 前後無音付き合成母音 → `f0_mean` == 合成に渡る fo のうち fo > 0 の要素の平均（1e-6 以内）
...
### SPEC-100 pitch で fo が変わる

再分解は harvest の `f0_floor=40` で行う（既定の 71Hz では pitch 0.5 の約 60Hz を追えずオクターブ誤りになるため）。

- **TC-100-1** 合成母音、pitch 1.5 で合成 → 再分解 fo 中央値 / 元の fo 中央値 が 1.455〜1.545
- **TC-100-2** pitch 0.5 → 比が 0.485〜0.515

### SPEC-101 無声フレームは 0 のまま

- **TC-101-1** 前後無音付き合成母音、pitch 2.0 → 合成に渡る fo で、元 fo == 0 のフレームがすべて 0、有声フレームは元の 2 倍
