...
- **005-ui-affordance による UI 変更後も、001 の保証は 86 件すべて維持されている。** 001 に属する結果行は 133 行で全て PASSED。
  前回検証（`3e94891`、213 passed）との差は 005 の 21 件の追加のみで、001 の結果行の数・内訳は変わっていない。
- 005 は画面から常時表示の説明文を取り除いたが、001 の E2E が観測点にしているセレクタ
  （`#rec` / `#play-orig` / `#play-mod` / `#frame` / `input[name=…]` / SVG の各 path）は残っており、
  セレクタが空振りして「素通しで PASS」になっているテストは見つからなかった（TC-230-1・TC-240-1・TC-244-2 などは
  値やイベントの結果まで検査している）。
- 005 は `document` の `keydown` で Escape を拾うリスナーを追加している。001 の SPEC-243/244（Space キーの A/B 切り替え）
  と競合していないことは、TC-243-1 / TC-243-2 / TC-244-1 / TC-244-2 の PASS で確認された。
