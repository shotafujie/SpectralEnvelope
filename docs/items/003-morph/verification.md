...
- 005 による UI 変更後も、003 の 23 仕様はすべて PASS。003 に属する結果行は 42 行で全て PASSED
  （TC-428-1 と TC-428-2 が 1 つのテスト関数 `test_TC_428_1_TC_428_2_リセットとプリセットはmixだけ戻す` に
  まとまっているため、行数 42 < TC 43）。
- 005 は `#current` / `#partner` / mix スライダーに `data-tip` を追加し、mix スライダーに
  `accent-color` の切り替え（`.changed`）を導入したが、003 の TC-420-1 / TC-424-1 / TC-429-1 は
  ラベル文字列・min/max/step/value・ダブルクリック後の値をそれぞれ検査しており、いずれも PASSED。
- SPEC-427 が要求する「元包絡・加工後包絡と異なる色の破線」は TC-427-1 が stroke の相違と
  `stroke-dasharray != none` で確認している。004 のゲインカーブ線（`#curve-line`）との色の相違は
  どの TC も検査していない（仕様が要求していないため判定には影響しない）。
- SPEC-430 の「11 件目で最古が消える」は TC-430-1 の 1 ケースのみで、相手に選ばれていない録音が消える経路は
  同じテストの中で一括して確認されている。
