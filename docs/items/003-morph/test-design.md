...
- **TC-420-1** 録音 1 回 + `voice.wav` 読み込み → `#current` の option が 2 件、`#partner` が 3 件（先頭が「なし」で value 空）。ラベルが順に「#1」「録音」「秒」、「#2」「voice.wav」「秒」を含む
...
- **TC-428-2** 相手選択 + mix 0.5 → プリセット「太く」→ mix 0、相手の選択は変わらない
