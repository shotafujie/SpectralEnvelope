...
### SPEC-1112 テストジョブは Node テストを実行する

- **TC-1112-1** テストジョブのいずれかのステップが `npm run test:js` を実行している
- **TC-1112-2** `npm run test:js` が、1 件でも失敗したとき 0 以外で終わる（失敗するテストを一時ファイルで足して確かめる）
