...
| `engine/world/wrapper.cpp` | C ラッパー。スパイクの約 40 行を元にする（長さ L で再合成して n に詰め直す処理は、JS のエンジン側で行う） | 新規 |
...
C ラッパーを変えたら、成果物を再ビルドして同じコミットに含める（ADR-0002）。長さ L の処理は JS 側で書いたので、タスク 8 ではラッパーを変えていない。
