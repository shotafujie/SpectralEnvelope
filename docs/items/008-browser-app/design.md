...
| `tests/e2e/conftest.py` | 静的配信のサーバーと、アダプタを包む観測点（`__calls` / `__hold`）を足す | 変更 |
| `tests/e2e/test_app_*.py` | 画面の E2E。v0.1.0 の判定をそのまま写し、観測点だけアダプタへ移す | 新規 |
...
v0.1.0 の E2E（`test_ui.py` ほか）は書き換えずに残す。
あれは凍結した `jig/` に対する 001〜005 の検証で、ブラウザ版の検証は 008 の ID で別に立てる。
同じ判定が二重に走るが、どちらも「その時点の実装が仕様を満たしているか」を答えているので嘘にはならない。
